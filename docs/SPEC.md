...
| F-003 | パスワードリセット | メールによるリセットフロー（実装済み: 2026-03-20） | 中 |
...
| F-015 | 通知履歴 | 過去の通知一覧表示（実装済み: 2026-03-20） | 低 |
...
| ID | 機能名 | 説明 | 優先度 |
|---|---|---|---|
| F-018 | カラーテーマ設定 | 株価の上昇/下落表示色をプリセットから選択（localStorage保存） | 中 |
| F-019 | 国際スタイル対応 | 「赤=上昇、緑=下落」（国際市場スタイル）の選択可能 | 中 |
...
- 株価データ更新: 1日1回（市場終了後）
- レコメンド生成: Redisキャッシュ（1時間TTL）、`asyncio.gather`による並列銘柄評価で高速化
- マーケット概況: Redisキャッシュ（30秒TTL）
